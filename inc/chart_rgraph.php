...
<?php
    ...
    while ($row = $result->fetch_array()) {
        ...
        echo "<div class='' style='margin-bottom:60px;'>";
            ...
            if ($showHumidity) {
                echo "
                <canvas class='linechart hidden-phone' id='$sensorID' width='990' height='400'>[No canvas support]</canvas>
                <script>
            ...
            $queryS = "SELECT AVG(temp_value), MAX(temp_value), MIN(temp_value), AVG(humidity_value), MAX(humidity_value), MIN(humidity_value) FROM ".$db_prefix."sensors_log WHERE sensor_id='$sensorID'";
            $resultS = $mysqli->query($queryS);
